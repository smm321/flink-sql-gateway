...
//		// properties rewrite, etc.
	...
	public ResultSet execute() {
		try {
			...
			for (int i = 0; i < queryOperation.getResolvedSchema().getColumnCount(); i++){
				...
				int finalI = i;
				Map<String, Tuple3<String, String, String>> element = new HashMap<>();
				origins.forEach(o -> {
					element.put(sinkColumn.get(finalI),
							Tuple3.of(o.getOriginTable().getQualifiedName().get(1),
	...
	private List<Tuple2<String, String>> findSourceTable(RelNode optRelNode, List list){
		...
		while (iterator.hasNext()) {
			return findSourceTable((RelNode)iterator.next(), list);
		}
		return list;
	}
}
