...
public class ValidateStatementSetOperation implements NonJobOperation {
	...
	public ResultSet execute() {
		try {
			...
			List<String> sqls = new CustomSqlParser().formatSql(statement);
			for (String sql : sqls) {
				for (Operation op : impl.getParser().parse(sql)) {
			...
			if (context.getExecutor() instanceof DefaultExecutor){
				...
				throw new TableException("Supported streaming mode only");
			}
